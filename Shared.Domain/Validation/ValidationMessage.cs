namespace Shared.Domain.Validation
{
    public class ValidationMessage
    {
        ...
        public static ValidationMessage CreateError(string translationKey, params object[] parameters)
        {
            ...
            return new ValidationMessage(SeverityLevel.Error, translationKey, parameters.Select(p => p.ToString()).ToArray() ?? []);
        }
        ...
        {
            if (Parameters.Any())
                return $"ValidationKey: {TranslationKey} Parameters: {string.Join(", ", Parameters).Replace("{", "{{").Replace("}", "}}")}";
